...
#		wp_enqueue_script( 'woo3dv-threejs-obj-loader2',  plugin_dir_url( __FILE__ ).'ext/threejs/js/loaders/OBJLoader2.js', array( 'jquery' ), $woo3dv_current_version );
		wp_enqueue_script( 'woo3dv-threejs-mtl-loader',  plugin_dir_url( __FILE__ ).'ext/threejs/js/loaders/MTLLoader.js', array( 'jquery' ), $woo3dv_current_version );
		wp_enqueue_script( 'woo3dv-threex',  plugin_dir_url( __FILE__ ).'ext/threex/THREEx.FullScreen.js', array( 'jquery' ), $woo3dv_current_version );
...
#	$detect = new Mobile_Detect;
	...
	$product_canvas_border = get_post_meta( get_the_ID(), '_product_canvas_border', true );
	...
	if (!woo3dv_is_woo3dv(get_the_ID())) return $image_url;
	$upload_dir = wp_upload_dir();
	$product_model = get_post_meta( get_the_ID(), '_product_model', true );
	$display_mode = get_post_meta( get_the_ID(), '_display_mode', true );
	$display_mode_mobile = get_post_meta( get_the_ID(), '_display_mode_mobile', true );
	...
	$product_background_transparency = get_post_meta( get_the_ID(), '_product_background_transparency', true );
	...
	if (get_post_meta( get_the_ID(), '_product_image_png', true )) $product_image_png = $upload_dir['baseurl'].'/woo3dv/'.get_post_meta( get_the_ID(), '_product_image_png', true );
	if (get_post_meta( get_the_ID(), '_product_image_gif', true )) $product_image_gif = $upload_dir['baseurl'].'/woo3dv/'.get_post_meta( get_the_ID(), '_product_image_gif', true );
	if (get_post_meta( get_the_ID(), '_product_video_webm', true )) $product_video_webm = $upload_dir['baseurl'].'/woo3dv/'.get_post_meta( get_the_ID(), '_product_video_webm', true );
...
/*		if (is_plugin_active('3dprint/3dprint.php')) {
			$conflicting_plugins[]='3DPrint';
		}
		if (is_plugin_active('3dprint-lite/3dprint-lite.php')) {
			$conflicting_plugins[]='3DPrint Lite';
		}
		if (is_plugin_active('vrm360/vrm360.php')) {
			$conflicting_plugins[]='Vrm360';
		}
		if (is_plugin_active('canvasio3d-light/canvasio3D.php')) {
			$conflicting_plugins[]='Canvasio3D light';
		}
		if (is_plugin_active('Canvasio3D/canvasio3D.php')) {
			$conflicting_plugins[]='Canvasio3D pro';
		}
		if (is_plugin_active('wp-3d-thingviewer-lite/wp-3d-thingviewer-lite')) {
			$conflicting_plugins[]='WP 3D Thingviewer Lite';
		}
		if (is_plugin_active('stl-viewer/stl.php')) {
			$conflicting_plugins[]='STL Viewer';
		}
		if (is_plugin_active('zem-stl-viewer/zemstl.php')) {
			$conflicting_plugins[]='ZeM STL';
		}
*/
